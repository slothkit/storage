...
## 在 Next.js（SSR）中的使用说明

- 本库在浏览器端会使用 `window.localStorage` 来持久化数据（这意味着数据是按浏览器 profile/域名隔离的，通常视为“当前用户的本地数据”）。
- 在服务端（例如 Next.js 的 SSR/Server-side 环境）没有 `window`，库会使用一个“no-op”回退实现：它不会持久化数据，也不会在服务器进程间或请求间共享数据。这样可以保证在服务器端导入库时不会抛错，也避免意外将用户数据共享到其它请求。

简短建议：对于大多数 Next.js 用例，保持默认的 no-op 服务端回退是最安全的做法；把真正的持久化交给浏览器 localStorage（客户端）或后端存储（按用户隔离）。
